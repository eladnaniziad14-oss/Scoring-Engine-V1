...
def fetch_btc_vol():
    ...
    data = safe_json(resp)
    if not data:
        logger.error("CoinGecko BTC data failed")
        return None
    ...
    out=pd.DataFrame([{"index":vol,"source":"BTC_VOL","timestamp":datetime.utcnow().isoformat()}])
    save_csv_parquet(out, "data/volatility/btc_vol")
    logger.info("Saved BTC volatility")
    return out
