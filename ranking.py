...
def add_selection_flags(
    ...
    *,
    top_pct: float = 0.30,
    score_col: str = "final_reliability_score",  # ✅ default to the new final score
    # Optional gates (set to None to disable)
    min_score: float | None = None,              # gate on score_col
    min_user_conf: float | None = 0.70,          # gate on df["user_confidence"]
    min_structural: float | None = 0.55,         # gate on df["structural_reliability"]
    min_entry_score: float | None = None,        # gate on df["entry_score"] (optional)
) -> pd.DataFrame:
    """
    ...
    1) Apply optional gates:
       ...
       - min_entry_score on df['entry_score'] (if present)
       - min_score on df[score_col]
    2) From remaining rows, select top_pct by score_col (descending).
    """
    out = df.copy()
    out["selected"] = False
    ...
    if min_entry_score is not None and "entry_score" in out.columns:
        mask &= out["entry_score"] >= float(min_entry_score)
